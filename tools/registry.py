...
# Security: Tools that should NEVER be auto-discovered in LeadFlow Agent Core.
# These tools provide dangerous capabilities (remote execution, file access)
# that could be exploited via prompt injection in a web-based agent backend.
_BLOCKED_TOOLS = frozenset({
    "terminal_tool",          # Remote command execution (RCE risk)
    "code_execution_tool",    # Code execution (RCE risk)
    "file_tools",             # File system read/write (data exfiltration risk)
    "file_operations",        # File-system backend helper
    "file_state",             # File mutation tracking helper
    "delegate_tool",          # Sub-agent delegation (privilege escalation)
    "browser_tool",           # Browser automation (SSRF risk)
    "browser_cdp_tool",       # Browser automation (SSRF risk)
    "browser_dialog_tool",    # Browser automation (SSRF risk)
    "browser_supervisor",     # Browser automation supervisor
    "computer_use_tool",      # Desktop automation
    "process_registry",       # Process management
    "cronjob_tools",          # Scheduled tasks (persistence risk)
})
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    module_names = [
        ...
        and path.stem not in _BLOCKED_TOOLS
