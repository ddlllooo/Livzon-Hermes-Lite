...
| `agent.create_automation_draft` | 创建自动化草案，返回待确认项 |
| `agent.preview_automation` | 校验自动化定义、能力版本和当前有效范围 |
| `agent.confirm_automation` | 确认并启用自动化，返回待确认项 |
...
- 自动化先由 `agent.preview_automation` 根据当前 Livzon 范围编译校验；草案、确认启用、修改、启停和归档均由后端确认链路控制。管理员平台查询仅获取脱敏载荷，不能借治理身份读取业务明细。
