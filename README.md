...
Hermes-Lite 保留 Hermes Agent 的通用 Agent 运行时、模型提供商接入、提示词组装、记忆支持、会话召回、Web 研究工具和工具注册机制，同时从默认工具集中移除了产品化业务层和高风险本地自动化能力。

这个仓库面向需要嵌入或二次扩展的轻量、Web 安全 Agent 核心；默认不包含商业情报、招投标匹配、数据库 RAG、浏览器自动化、终端执行或媒体生成能力。
...
- 招标搜索、招标推荐、招标匹配、招标分析
- 六维度投标匹配和中标概率评分
- 基于 OceanBase 的知识库/RAG 管道
- 业务示例数据和数据库初始化脚本
- 终端、文件修改、代码执行、进程、定时任务、子代理、浏览器、桌面自动化、图片/视频/TTS 等工具的自动发现
